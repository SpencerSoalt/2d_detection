...
class YOLOv12BatchDetectionNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('image_size', 320)  # Input image size for inference
        self.declare_parameter('num_cores', 6)  # Number of CPU cores (0 = auto)
        ...
        num_cores = self.get_parameter('num_cores').value
        
        # Set number of CPU threads
        if num_cores > 0:
            torch.set_num_threads(num_cores)
            self.get_logger().info(f'Set PyTorch to use {num_cores} CPU threads')
        else:
            self.get_logger().info(f'Using default CPU thread count: {torch.get_num_threads()}')
        ...
        self.get_logger().info(f'Input image size for inference: {self.image_size}')
        ...
        self.pub1 = self.create_publisher(
            Detection2DArray, '/camera1/detections', image_qos)
        self.pub2 = self.create_publisher(
            Detection2DArray, '/camera2/detections', image_qos)
    ...
    def process_batch(self):
        ...
        # Run batch inference with class filtering and image size
        results = self.model(
            images, 
